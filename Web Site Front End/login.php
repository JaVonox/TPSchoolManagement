...
<?php
            ...
            if (!isset($_SESSION['Login']) && !empty($_POST['UserID']) 
               && !empty($_POST['Password'])) 
			{
				...
				else
				{
					...
					$query = "SELECT Password FROM Login where Person_ID = ?";
					$statement = $connection->prepare($query);
					// Now execute the query proving parameters for the placeholders
					$statement->execute(array('?' => $our_ID));
				    ...
				    if ($our_Password == $result["Password"]) 
				    {
					    ...
					    $query = "SELECT Student_ID FROM Student where Person_ID = ?";
						$statement = $connection->prepare($query);
						// Now execute the query proving parameters for the placeholders
						$statement->execute(array('?' => $our_ID));
						$result = $statement->fetch(PDO::FETCH_ASSOC);
